...
void tftp_send_ack(u16 blocknum)
{
	...
	//UDP头部信息
	udphdr->sport = HON(48915);
	udphdr->dport = HON(severport); //此为响应包，这应该回复到相应端口；比较请求包
	udphdr->len = HON(tftp_len + sizeof(UDP_HDR) - sizeof(IP_HDR));//UDP协议报文中含有IP报文，可查其定义的数据 结构
	udphdr->udpchksum = 0x00;
...
{
	u32 i;
	u32 tftp_len; //定义tftp包的长度
	severport = port;
	
	TFTP_PAK *ptftp = buf + sizeof(UDP_HDR);//指向tftp数据报文部分
	
	tftp_len = len -sizeof(UDP_HDR);
	
	if(HON(ptftp->opcode) == 3)//数据包的编号为3
	{
		if(HON(ptftp->blocknum) == curblock)//防止重复接收块号相同的数据包
		{
		
			tftp_down_addr = (u8*)(SDRAM_KERNEL_START + (curblock-1)*512);
			for(i = 0; i<(tftp_len - 4); i++)//查看tftp数据包的格式知前四字节为操作码和块编号
			{
				*(tftp_down_addr + 1) = *(ptftp->data+i);
				
			}
			
			tftp_send_ack(HON(ptftp->blocknum));//接收到数据之后则回复响应包
			curblock++;
			
			if((tftp_len-4) < 512)//最后一个包一定小于512 ;复位标志块标号和内存起始地址，为下一次传输做准备
			{
				curblock = 1;
