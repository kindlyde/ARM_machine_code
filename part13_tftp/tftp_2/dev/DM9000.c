...
void dm_init(void)
{
	...
	/* Activate DM9000 */
	/* RX enable */
	dm9000_reg_write(DM9000_RCR, RCR_DIS_LONG |RCR_ALL |RCR_DIS_CRC | RCR_RXEN);
...
u32 dm9000_rx(u8 *data)
{
	...
	/* 5、读取包的数据--与以太网的数据的最大长度（ PKT_MAX	 1522）作比较，不可超过，注意接收的数据宽度为16位 */
	if(len < PKT_MAX_LEN)
	{
		for(i=0; i < len; i++)
		{
...
void dm_isr(void)
{
	...
	net_handle(&buffer[0],packet_len);//传递的第一个参数是一个以太网的包，包含IP包、ARP包
